...
exports.connect = exports.createSession = function(url, listener) {
	...
	if (arguments.length > 1 && typeof listener != 'function') {
		...
		options = parse(url);
		options = options.slashes ? options.hostname : url;
		...
		if (options.url) {
			...
			options.host = url.hostname ? url.hostname : options.url;
			options.port = options.port ? options.port : url.port;
			options.tls = url.protocol == 'ssmpp:';
			if (options.bind) options.localAddress = options.bind;
		}
	}
